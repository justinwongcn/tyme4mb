...
│  命理网站 / App / 日历工具 / 数据分析                 │
└──────────────────────┬──────────────────────────────┘
                       │ 调用 @tyme.* (via reexports)
┌──────────────────────▼──────────────────────────────┐
│              Facade 层：兼容入口                      │
│  reexports.mbt — 重导出所有公共类型                   │
│  保持 @tyme.* 向后兼容，无需修改导入路径               │
└──────────────────────┬──────────────────────────────┘
                       │ 依赖
┌──────────────────────▼──────────────────────────────┐
│              领域层：时间计算 (tyme/core/)             │
...
9. **Facade 兼容层**：`tyme/reexports.mbt` 通过 `pub using @core {...}` 和 `pub using @base {...}` 保持 `@tyme.*` 命名空间向后兼容，消费者无需修改导入路径。
10. **天文算法分离**：纯天文计算（节气、朔望、真太阳时）独立为 `tyme/astronomy/` 包，避免领域类型依赖。